...
class Student extends Backend
{
    ...
    public function _initialize()
    {
        ...
        $this->view->assign("vipLevelList", $this->model->getVipLevelList());
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                    ->with(['school','subject','up','college','graduated'])
                    ->where($where)
                    ->order($sort, $order)
                    ->count();
            ...
            foreach ($list as $row) {
				...
				$row->getRelation('up')->visible(['name']);
				$row->getRelation('college')->visible(['name']);
				$row->getRelation('graduated')->visible(['name']);
            }
